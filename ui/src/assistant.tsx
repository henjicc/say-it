...
export function AssistantAnswerApp() {
  const [answer, setAnswer] = useState<Answer>({ text: "", reasoning: "", sourceText: "", canInsert: false, streaming: false });
  ...
  return <div className="assistant-answer-window flex flex-col overflow-hidden text-[var(--color-fg)]">
    <header data-tauri-drag-region className="flex h-12 flex-none items-center border-b border-[var(--color-line)] px-4"><strong data-tauri-drag-region className="text-sm">智能助手</strong><button type="button" className="ml-auto rounded-[var(--radius-sm)] p-2 hover:bg-[var(--color-surface)]" aria-label="关闭" onClick={() => void cmd(CMD.closeAssistantAnswer)}><X className="h-4 w-4" /></button></header>
    <main className="min-h-0 flex-1 overflow-y-auto p-5">
      ...
      {answer.error ? <p className="text-sm text-[var(--color-err)]">{answer.error}</p> : answer.text ? <SafeMarkdown text={answer.text} /> : <p className="text-sm text-[var(--color-fg-subtle)]">{answer.streaming ? "正在生成回答…" : "正在等待回答…"}</p>}
    </main>
    <footer className="flex flex-none items-center gap-2 border-t border-[var(--color-line)] p-3"><Button size="sm" onClick={() => void copy()} disabled={!answer.text || answer.streaming}><Copy className="h-4 w-4" />复制</Button><Button size="sm" onClick={() => void regenerate()} disabled={busy || answer.streaming}><RefreshCw className="h-4 w-4" />重新生成</Button>{answer.canInsert && <Button size="sm" variant="primary" onClick={() => void cmd(CMD.insertAssistantAnswer)}><CornerDownLeft className="h-4 w-4" />插入当前位置</Button>}<span role="status" className="ml-auto text-xs text-[var(--color-fg-subtle)]">{message}</span></footer>
  </div>;
}
