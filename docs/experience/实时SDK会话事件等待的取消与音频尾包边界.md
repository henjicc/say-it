...
应用级混合模型验收发现：5 秒 16kHz 单声道输入，经实时 DSP 后只收到 159,680 字节，预期为 160,000 字节。通道尾包和文件解码完整并不意味着 DSP 内部已经排空：线性插值等待下一样本，且处理器会缓存不足 480 个 48kHz 样本的一帧；原会话在 Finish 时直接结束识别，没有处理缓存。

正常 Finish 必须先调用 `StreamDsp::finish` 并把结果送入识别器，再关闭识别输入。重采样末端按末值延展，残帧补零仅用于既有降噪、EQ、增益计算；输出按原输入折算到 16kHz 的样本数裁剪，不能为凑整帧延长时长。取消和失败不执行这一步，重复结束不重发尾部。内置 SDK、JavaScript 插件、本地模型和 Apple 输入均遵守同一顺序。

此修复保留原有逐帧信号处理及其算法延迟，不改降噪算法。回归覆盖八种采样率（含默认值）、短于一帧的输入、整帧、分包、降噪开关、重复结束，以及与冻结旧实现的已输出音频逐字节一致性。Windows 完整 Rust release 回归 717 项通过、32 项显式忽略；Apple 接入仅代码检查，仍需 macOS 环境运行验证。

隔离应用 `comparison-app-3` 完成三轮文件模型成功/取消/失败/再次成功，以及文件与实时模型混合对比；实时插件严格收到 160,000 字节，原 159,680 字节回归失败已消失。使用本地无网络插件，只验证宿主音频和生命周期，不代表真实识别质量或供应商延迟。应用退出后无残留子进程。
