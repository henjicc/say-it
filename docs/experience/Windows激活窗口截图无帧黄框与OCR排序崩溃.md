...
- 听写结束时解析上下文前，先用 `oneshot::Receiver::try_recv` 读取已完成的捕获结果；不能先按捕获截止时间返回超时。用户可能持续听写超过 5 秒，而 OCR 已在此前完成，先检查截止会错误丢弃成功结果并向 LLM 传入空上下文。
- 捕获句柄被取消、超时或会话替换时，应通过共享原子标记让截图/OCR 等待层尽快退出；OCR 工作线程在真正推理前再次检查该标记和绝对截止时间，跳过排队的过期任务。MNN 正在执行的单次推理不可安全抢占，但不应继续占用捕获名额或让后续过期任务推理。
