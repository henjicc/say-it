...
- 字幕悬浮窗和 OBS 覆盖页消费已保存字体时必须追加 `PingFang SC`、`Microsoft YaHei` 与通用无衬线回退，避免从 Windows 带来的缺失字体在 macOS 上退回衬线体。只为 WebView2 菜单键问题注册的全局 Alt `preventDefault` 必须限制在 Windows；macOS 的 Option 是正常文本输入修饰键，不能在字幕编辑器中全局吞掉。
...
- macOS LaunchAgent 的 `Label` 和 plist 文件名应使用稳定、唯一的反向域名标识，不能直接继承本地化 `productName`。从旧名称迁移时只能由 `.app/Contents/MacOS/` 内的正式应用执行，开发态 `target/debug` 二进制不能把已安装应用的自启项改写到开发目录。
- Unix（包括 macOS）保存主配置时应直接用同目录 `rename` 原子替换目标，不能为了兼容 Windows 先删除旧文件，否则掉电或进程终止会留下“主文件缺失、备份仍在”的中间状态。读取时也必须把仅存在 `.bak` 视为可恢复状态，不能误判为首次启动并覆盖用户设置。
