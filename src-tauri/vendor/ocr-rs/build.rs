...
fn main() {
    ...
    let (mnn_include_dir, mnn_lib_dir) = match &link_mode {
        ...
        MnnLinkMode::Dynamic | MnnLinkMode::Static => {
            if mnn_static && env::var("MNN_LIB_DIR").is_err() && os == "windows" && arch == "x86_64"
            {
                let asset_name = get_prebuilt_asset_name(&os, &arch)
                    .expect("No bundled static MNN asset for this target");
                let prebuilt_dir = download_prebuilt_mnn(&manifest_dir_path, &asset_name, &os);
                println!(
                    "cargo:warning=Using bundled checksum-pinned static MNN from: {}",
                    prebuilt_dir.display()
                );
                (
                    vec![prebuilt_dir.join("include")],
                    vec![prebuilt_dir.join("lib")],
                )
            } else {
...
fn download_prebuilt_mnn(manifest_dir: &Path, asset_name: &str, os: &str) -> PathBuf {
    ...
    // The Windows x64 archive is bundled and checksum-pinned so release builds
    // never depend on the mutable upstream `dev` asset.
    if !archive_path.exists() {
        let bundled_archive = manifest_dir
            .join("vendor")
            .join(format!("{}.{}", asset_name, ext));
        if os == "windows" && asset_name == "mnn-dev-windows-x86_64" {
            verify_archive(&bundled_archive, WINDOWS_X64_ARCHIVE_SHA256);
            fs::copy(&bundled_archive, &archive_path).unwrap_or_else(|error| {
                panic!(
                    "Failed to copy bundled MNN archive {}: {}",
                    bundled_archive.display(),
                    error
                )
            });
            println!(
                "cargo:warning=Using checksum-pinned bundled MNN archive: {}",
                bundled_archive.display()
            );
        } else {
...
fn verify_archive(path: &Path, expected: &str) {
    let bytes = fs::read(path)
        .unwrap_or_else(|error| panic!("Failed to read MNN archive {}: {}", path.display(), error));
    let actual = format!("{:X}", Sha256::digest(bytes));
    if actual != expected {
        panic!(
            "MNN archive checksum mismatch for {}: expected {}, got {}",
            path.display(),
            expected,
            actual
        );
