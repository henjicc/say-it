...
html,body,#root{width:100%;height:100%;margin:0;overflow:hidden;background:transparent}body{font-family:"PingFang SC","Microsoft YaHei",sans-serif}#root{box-sizing:border-box;display:flex;align-items:center;justify-content:center}.stack{width:100%;display:flex;flex-direction:column;align-items:center;gap:10px}.caption{box-sizing:border-box;width:100%;max-width:100%;overflow:hidden;padding:10px 22px;border:1px solid rgba(255,255,255,.1);text-align:center;word-break:break-word;font-weight:600;line-height:1.38}.caption.empty{display:none}.flow{position:relative;overflow:hidden}.content{position:absolute;left:0;right:0;top:0;white-space:pre-wrap;overflow-wrap:anywhere;transform:translate3d(0,0,0);transition:transform var(--motion-duration,120ms) var(--motion-easing,ease-out);will-change:transform}.caption.replace .content{right:auto;white-space:nowrap}.caption.no-motion .content{transition:none}.fresh{animation:fresh-in var(--fade-duration,180ms) var(--fade-easing,ease-out) both}@keyframes fresh-in{from{opacity:0}to{opacity:1}}
...
mod tests {
    ...
    #[test]
    fn overlay_page_keeps_macos_and_windows_font_fallbacks() {
        assert!(OVERLAY_PAGE.contains("fontStack(style.fontFamily)"));
        assert!(OVERLAY_PAGE.contains("PingFang SC"));
        assert!(OVERLAY_PAGE.contains("Microsoft YaHei"));
    }
}
